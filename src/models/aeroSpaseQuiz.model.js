...
class aeroSpaseQuiz {
    ...
    static async questions(lesson,lang) {
        try {
            ...
            switch (lang) {
              case "AM":
                query = pg("questions").select("*").orderBy("id").where('lesson','=',lesson).returning("*");
                break;
              case "US":
                query = pg("questions_en").select("*").orderBy("id").where('lesson','=',lesson).returning("*");
                break;
              case "RU":
                query = pg("questions_ru").select("*").orderBy("id").where('lesson','=',lesson).returning("*");
                break;
              default:
                query = null;
            }
