...
class aeroSpaceQuestionsModel {
  static async getSatelliteQuestions(lang) {
    try {
      return lang === "AM"
        ? pg("satellite_questions").select("*").orderBy("id").returning("*")
        : lang === "US"
        ? pg("satellite_questions_en").select("*").orderBy("id").returning("*")
        : lang === "RU"
        ? pg("satellite_questions_ru").select("*").orderBy("id").returning("*")
        : null;
    } catch (error) {
      console.log(error);
